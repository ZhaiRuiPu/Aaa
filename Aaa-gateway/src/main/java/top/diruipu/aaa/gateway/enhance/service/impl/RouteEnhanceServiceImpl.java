...
import top.diruipu.aaa.gateway.enhance.entity.*;
import top.diruipu.aaa.gateway.enhance.service.*;
import top.diruipu.aaa.gateway.enhance.utils.AddressUtil;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
    ...
    public Mono<Void> filterBlackList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                String requestIp = SystemUtil.getServerHttpRequestIpAddress(request);
                ...
                if (forbid.get()) {
                    return SystemUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                            HttpStatus.NOT_ACCEPTABLE, new AaaResponse().message("黑名单限制，禁止访问"));
                }
            } else {
    ...
    public void saveRequestLogs(ServerWebExchange exchange) {
        ...
        if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
            ...
            ServerHttpRequest request = exchange.getRequest();
            String ipAddress = SystemUtil.getServerHttpRequestIpAddress(request);
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                return SystemUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                        HttpStatus.TOO_MANY_REQUESTS, new AaaResponse().message("访问频率超限，请稍后再试"));
            } else {
